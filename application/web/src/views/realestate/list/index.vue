<template>
  <div class="container">
    ...
    >
      ...
      <p>After the sale, donation or pledge operation is initiated, the guarantee status is true</p>
      <p>When the guarantee status is false, the sale, donation or pledge operation can be initiated</p>
    </el-alert>
    ...
    <el-dialog v-loading="loadingDialog" :visible.sync="dialogCreateSelling" :close-on-click-modal="false" @close="resetForm('realForm')">
      <el-form ref="realForm" :model="realForm" :rules="rules" label-width="100px">
        <el-form-item label="Price (INR)" prop="price">
          <el-input-number v-model="realForm.price" :precision="2" :step="10000" :min="0" />
        </el-form-item>
        <el-form-item label="Validity Period ()" prop="salePeriod">
  ...
  methods: {
    ...
    createSelling(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm('Do you want to sell now?', 'Hint', {
    ...
    createDonating(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm('Do you want to donate now?', 'Hint', {
            confirmButtonText: 'Sure',
            cancelButtonText: 'Cancel',
            type: 'success'
          }).then(() => {
